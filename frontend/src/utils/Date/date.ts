...
export const getDaysOfMonth = (year: number, month: number) => {
  if (month <= 0 || month > 12)
    throw `Get ${month}. This number matches no month`;
...
export const getMonthShortcut = (month: string) =>
  `${month.length <= 4 ? month : month.substring(0, 4) + "."}`;
...
export const getNumberOfDaysBeetween = (start: Date, end: Date) => {
  ...
  let diffInDays = Math.floor((endUTC - startUTC) / oneDay);
